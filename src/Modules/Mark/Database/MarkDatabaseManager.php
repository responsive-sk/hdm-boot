...
namespace HdmBoot\Modules\Mark\Database;
...
final class MarkSqliteDatabaseManager extends AbstractDatabaseManager
{
    ...
    protected function createConnection(): PDO
    {
        try {
            ...
            throw new RuntimeException('Failed to connect to mark database: ' . $e->getMessage(), 0, $e);
        }
    }
     ...
     */
    private function createDefaultMarkUser(): void
    {
    ...
    {
        $stmt = $this->connection->prepare("DELETE FROM mark_sessions WHERE expires_at < datetime('now')");
        $stmt->execute();
