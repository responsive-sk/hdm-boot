...
namespace HdmBoot\Modules\Core\App\Database;
...
/**
 * App SQLite Database Manager - Handles app.db SQLite database.
 *
 * ARCHITECTURE PRINCIPLE: App database is for CORE SYSTEM modules only.
 * This database contains ONLY core system data (Cache, System logs, Template cache).
 *
 * NOTE: Blog is an OPTIONAL module and should have its own blog.db database.
 * This implementation includes Blog tables for backward compatibility only.
 */
final class AppSqliteDatabaseManager extends AbstractDatabaseManager
{
    public function __construct(
        string $databasePath = 'storage/app.db',
        ?Paths $paths = null
    ) {
    ...
    /**
     * Create PDO connection to app.db.
     */
    protected function createConnection(): PDO
    {
        try {
            ...
            throw new RuntimeException('Failed to connect to app database: ' . $e->getMessage(), 0, $e);
        }
    }
    ...
    protected function initializeDatabase(): void
    {
        try {
            ...
            $connection->exec("CREATE INDEX IF NOT EXISTS idx_app_cache_expires ON app_cache(expires_at)");
     ...
     */
    private function createSampleBlogArticle(): void
    {
