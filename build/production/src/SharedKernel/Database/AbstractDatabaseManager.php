...
abstract class AbstractDatabaseManager
{
    ...
    {
        if ($this->paths !== null) {
            // If path is already absolute (from $paths->storage()), return as-is
            if (str_starts_with($path, '/') || (PHP_OS_FAMILY === 'Windows' && preg_match('/^[A-Z]:\\\\/', $path))) {
                return $path;
            }
            // Use Paths service for relative path resolution
