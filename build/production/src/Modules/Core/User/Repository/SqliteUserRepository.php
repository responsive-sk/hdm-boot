...
final class SqliteUserRepository implements UserRepositoryInterface
{
    ...
    {
        try {
            error_log('🔍 USER REPO DEBUG: Looking for user: ' . $email);
            error_log('🔍 USER REPO DEBUG: PDO connection: ' . ($this->pdo ? 'OK' : 'NULL'));
            ...
            error_log('🔍 USER REPO DEBUG: Query result: ' . ($data ? 'FOUND' : 'NOT_FOUND'));
            if ($data) {
                error_log('🔍 USER REPO DEBUG: User data: ' . json_encode($data));
            }

            return $data ?: null;
        } catch (\Exception $e) {
            error_log('🔍 USER REPO DEBUG: Exception: ' . $e->getMessage());
            throw $e;
        }
    }
